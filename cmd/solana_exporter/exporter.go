...
import (
	"context"
	"flag"
	"git.reddfive.com/staking-protocol-engineering/protocols/sol/solana-exporter/pkg/rpc"
...
var (
	...
	rpcAddr = flag.String("rpcURI", "https://api.mainnet-beta.solana.com", "Solana RPC URI (including protocol and path)") // for testing locally
...
func NewSolanaCollector(rpcAddr string) *solanaCollector {
	return &solanaCollector{
		...
		solanaVersion: prometheus.NewDesc(
			...
			[]string{"version"}, nil),
		blockTimeDesc: prometheus.NewDesc(
			"solana_block_time",
...
func (c *solanaCollector) Collect(ch chan<- prometheus.Metric) {
	...
	currentSlot, err := c.rpcClient.GetCurrentSlot(context.Background())
    if err != nil {
        // Handle error
        klog.Errorf("Failed to get current slot: %v", err)
        return
    }

    // Fetching block time for the current slot
    blockTime, err := c.rpcClient.GetBlockTime(context.Background(), currentSlot)
    if err != nil {
        ch <- prometheus.NewInvalidMetric(c.blockTimeDesc, err)
    } else {
        ch <- prometheus.MustNewConstMetric(
            c.blockTimeDesc,
            prometheus.GaugeValue,
            float64(blockTime),
            fmt.Sprintf("%d", currentSlot),
        )
    }
